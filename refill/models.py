...
class NewRxAsQeust(models.Model):
    phone_regex = RegexValidator(regex=r'^\+?1?\d{9,15}$',
                                 message="Phone number must be entered in the format: '+999999999'. Up to 15 digits allowed.")
    ...
    phone_number = models.CharField(validators=[phone_regex], max_length=17, null=False, blank=True)
    #Date_of_Birth= models.DateField()
    created = models.DateTimeField(auto_now_add=True)
    delivered=models.BooleanField(default=False)
    updated = models.DateTimeField(auto_now_add=True)
    ...
    Dr_name = models.CharField(max_length=50, help_text="only required if verifying with dr is selected", blank=True)
    dr_phone_number = models.CharField(validators=[phone_regex], max_length=17, blank=True,null=True)
    Dr_adrress = models.CharField(max_length=100, blank=True)
    ...
    def save(self):
        ...
        while a == 1:
            code = 'rx{}'.format(random.randrange(1000, 100000))
            if not NewRxAsQeust.objects.filter(rx_number=code).exists():
                self.rx_number = code
                a = 2
        super(NewRxAsQeust, self).save()

class DrugQeust(models.Model):
    med=models.ForeignKey(NewRxAsQeust,null=True,related_name='drug_qeust')
    drug_name = models.CharField(max_length=100,blank=True)
    drug_dose = models.CharField(max_length=20,blank=True)
    drug_price = models.DecimalField(max_digits=6, decimal_places=2, blank=True, null=True,default=0)
